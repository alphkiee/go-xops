...
func GetNameSpaces(ctx context.Context) ([]k8sm.NameSpace, error) {
	result := []k8sm.NameSpace{}
	data, err := common.ClientSet.CoreV1().Namespaces().List(ctx, v1.ListOptions{})
	if err != nil {
		return nil, err
	}
	for _, namespace := range data.Items {
		result = append(result, k8sm.NameSpace{
...
func GetNamespace(ctx context.Context, name string) (k8sm.NameSpace, error) {
	result := k8sm.NameSpace{}
